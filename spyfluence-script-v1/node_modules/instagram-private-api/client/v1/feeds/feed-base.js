...
FeedBase.prototype.all = function (parameters) {
    ...
    _.defaults(parameters, { delay: 3000 , every: 200, pause: 30000, maxErrors : 10, limit: this.limit });
    ...
    return Promise.delay(delay)
        ...
        .catch(Exceptions.ParseError, function () {
            // Every consecutive ParseError makes delay before new request longer. Otherwise we will never reach the end.
        ...
        .then(function (response) {
            var results = response[0].map(that.map);

            if(_.isFunction(that.reduce))
                that.allResults = that.reduce(that.allResults, results);
            that.totalCollected += response[0].length;

            that._handleInfinityListBug(response[0], results);

            that.emit('data', results);
            var exceedLimit = false;

            // console.log('limit: ', parameters.limit);
            // console.log('collected ', that.totalCollected);
            ...
            if (that.isMoreAvailable() && !exceedLimit) {
                that.iteration++;
                that.cursors[(that.allResults.length - 1)] = response[1]
                return that.all(parameters);
            } else {
                that.iteration = 0;

                that.emit('end', that.allResults);
                return [that.allResults, that.cursors];
            }
        })
