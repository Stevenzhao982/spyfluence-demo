...
Rails.application.config.assets.precompile += Dir[Rails.root.join('app', 'assets', 'images', '*')].map {
  |file| file.sub(%r(#{Rails.root.join('app', 'assets', 'images/*')}), '')
}
...
Rails.application.config.assets.precompile += ['javascripts/pages/*.js']
